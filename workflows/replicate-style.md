...
> Extract the reference frames LOCALLY (ffmpeg) — the MCP no longer serves raw-asset frames; download/extract on your machine and read the PNGs.
...
1. **Acquire material** (split by source)
   ...
   - Project asset: download it locally (or keep the original) and extract frames with ffmpeg — frame extraction is local now
...
5. **Iterate against the reference**: `render_shot(id, storyboard=true)` → read `get_state()` → `shots[].render.still` **side-by-side** with the reference frame strip → tune parameters and re-render. Check common gaps in order: easing type → duration → stagger → font size/tracking.
