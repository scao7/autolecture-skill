...
> **开放/封闭分层（2026-06）**：`list_gallery_templates` / 搜索 **只列官方
> （curated）题材**——模糊浏览永远只命中审核过的那批，用户的意图不会落到一份
> 没校验的社区配方上。用户**自己发布**的社区模版**不在这个列表里**，只能用
> **精确 slug** 直接 `get_template_card(slug)` / `use_gallery_template(slug)`
> 加载（这两个工具解析**任意** id，官方或社区都行）。所以：当用户报出一个具体
> 的社区模版名/链接时，**别去模糊匹配**——问出（或回忆）它的确切 slug，按 id 直接拉。
...
> 校验编译 → 上架）。**注意开放/封闭分层**：用户发布的是**社区模版**——
> `official=false` 且 `unlisted`，**只能按确切 slug 加载，不进模糊浏览列表**
> （把 slug 给用户收好）；上架到官方 curated 目录是另一条（seed/admin）路径，
> 不经此端点。见 `publish_template`（服务端提供该工具时启用）。
